...
const char * const DIGIBYTE_CONF_FILENAME = "bitscoin.conf";
const char * const DIGIBYTE_PID_FILENAME = "bitscoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitscoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitsCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitsCoin
    // Mac: ~/Library/Application Support/BitsCoin
    // Unix: ~/.bitscoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitsCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitsCoin";
#else
    // Unix
    return pathRet / ".bitscoin";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure BitsCoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("BitsCoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The BitsCoin Core developers";
    }
