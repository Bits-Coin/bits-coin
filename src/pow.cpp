...
unsigned int GetNextWorkRequiredV1(const CBlockIndex* pindexLast, const Consensus::Params& params, int algo)
{
	...
	// BitsCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits, const Consensus::Params& params)
{
	...
	//error("Error is here : Inside CheckProofOfWork Function ");
 // Check range
    if (fNegative || bnTarget == 0 || fOverflow || bnTarget > UintToArith256(params.powLimit))
	{
		//error("Error is here 1");
        return false;
	}
    // Check proof of work matches claimed amount
    if (UintToArith256(hash) > bnTarget)   
	{
		//error("Error is here  Inside this Conidtion UintToArith256(hash) > bnTarget");
        return false; 
	}		
