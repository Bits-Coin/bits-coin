 ...
 * between bitscoind, and bitscoin-qt and non-server tools.
*/
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bitscoin/bitscoin>";
    const std::string URL_WEBSITE = "<https://bitscoin.io>";
...
/** Sanity checks
 *  Ensure that BitsCoin is running in a usable environment with all
...
{
    // Make sure only a single BitsCoin process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if bitscoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if bitscoin is started while in a temporary directory.\n",
