...
###   bitscoincore.org and bitscoin.org and compares them.
...
WORKINGDIR="/tmp/bitscoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitscoincore.org"
HOST2="https://bitscoin.io"
BASEDIR="/bin/"
VERSIONPREFIX="bitscoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitscoin-',
...
if [ $? -ne 0 ]; then
   echo "bitscoin.org failed to provide signature file, but bitscoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitscoin.org and bitscoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitsCoin Core binary release signing key installed?"
   fi
