#!/bin/bash
# Copyright (c) 2013 The BitsCoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete BitsCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no BitsCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old BitsCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected BitsCoin 0.7 datadir."
    ;;
  3)
    echo "Detected BitsCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected BitsCoin 0.8 datadir."
    ;;
esac
