...
The RPM spec file provided here is for BitsCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitscoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[BitsCoin git repository](https://github.com/bitscoin/bitscoin) but are not part of
...
Currently only `Source10` is used, for the example `bitscoin.conf` file.
...
in the bitscoin git repository. At present only an SVG version of the BitsCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitscoin
...
needed when 0.12.1 is released, a proper fix is already in the BitsCoin git
master branch.
...
idea to build BitsCoin Core against a static version of that library compiled
...
The `bitscoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitscoin-server` package. At this point, most RPM based
...
The `bitscoin-server` package should have SELinux support. How to properly do
...
BitsCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitscoin/). He has been
packaging BitsCoin for Fedora at least since 2012.
...
4. I renamed the `bitscoin` package that contains the Qt GUI to `bitscoin-core` as
that appears to be how the general population refers to it, in contrast to
`bitscoin-xt` or `bitscoin-classic`. I wanted to make sure the general population
